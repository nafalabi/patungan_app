...
import (
	...
	plan_pages "patungan_app_echo/internal/modules/admin/plan/pages"
...
func NewPlanHandler(db *gorm.DB, cache *cache.RedisCache) *PlanHandler {
...
func (h *PlanHandler) ListPlans(c echo.Context) error {
	...
	return plan_pages.PlansList(props).Render(c.Request().Context(), c.Response())
}
...
func (h *PlanHandler) CreatePlanPage(c echo.Context) error {
	...
	return plan_pages.PlanForm(props).Render(c.Request().Context(), c.Response())
}
...
func (h *PlanHandler) DeletePlan(c echo.Context) error {
	...
	if err != nil {
		return echo.NewHTTPError(http.StatusBadRequest, "Invalid plan plan.ID")
	}
...
func (h *PlanHandler) SchedulePlan(c echo.Context) error {
	...
	createdTask, err := ProcessPlanScheduleTask.CreateTask(taskArgs)
	if err != nil {
